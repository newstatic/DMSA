...
class NativeSyncEngine: ObservableObject {
        ...
        do {
            ...
            if config.enablePauseResume,
               ...
               savedState.isResumable {
                Logger.shared.info("检测到可恢复的同步状态，从断点继续")
            ...
            if plan.summary.isEmpty {
                Logger.shared.info("无需同步: \(task.syncPair.id)")
    ...
    private func scanPhase(task: SyncTask) async throws -> (DirectorySnapshot, DirectorySnapshot) {
        ...
        Logger.shared.info("开始扫描: \(task.syncPair.id)")
        ...
        Logger.shared.info("扫描完成: 源 \(sourceSnapshot.fileCount) 文件, 目标 \(destSnapshot.fileCount) 文件")
    ...
    private func diffPhase(
        ...
        Logger.shared.info("差异计算完成: \(diffResult.summary)")
    ...
    private func resolveConflictsPhase(plan: SyncPlan) async throws -> SyncPlan {
        ...
        Logger.shared.info("开始解决 \(plan.conflicts.count) 个冲突")
    ...
    private func syncPhase(plan: SyncPlan) async throws -> FileCopier.CopyResult {
        ...
        Logger.shared.info("开始同步: \(plan.totalFiles) 文件, \(ByteCountFormatter.string(fromByteCount: plan.totalBytes, countStyle: .file))")
        ...
        if !plan.conflicts.isEmpty {
            ...
            Logger.shared.info("冲突解决执行完成: \(conflictResult.summary)")
        }
        ...
        for file in filesToLock {
            if lockManager.acquireLock(file.virtualPath, direction: file.direction, sourcePath: file.sourcePath) {
                ...
                Logger.shared.debug("获取同步锁: \(file.virtualPath)")
            } else {
                Logger.shared.warn("无法获取同步锁，文件可能正在被其他操作使用: \(file.virtualPath)")
            }
        }
        ...
        Logger.shared.info("同步执行完成: 成功 \(result.succeeded), 失败 \(result.failed.count)")
    ...
    private func releaseAllLocks() {
        ...
        for path in pathsToRelease {
            ...
            Logger.shared.debug("释放同步锁: \(path)")
        }

        if !pathsToRelease.isEmpty {
            Logger.shared.info("已释放 \(pathsToRelease.count) 个同步锁")
        }
    }
    ...
    private func verifyPhase(plan: SyncPlan) async throws -> Int {
        ...
        Logger.shared.info("验证完成: \(filesToVerify.count) 文件, \(failures) 失败")
    ...
    private func resumeSync(
        ...
        Logger.shared.info("从断点恢复: 已完成 \(state.completedActionIndices.count)/\(state.plan.actions.count)")
